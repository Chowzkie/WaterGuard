...
    // MODIFIED: This hook now sends sensor readings to the backend for evaluation.
    useEffect(() => {
        if (mockSensorReadings.length === 0) return; // Wait until mock readings are fetched

        let readingIndex = 0;
        const intervalId = setInterval(async () => { // Made async to await axios.post
            if (readingIndex < mockSensorReadings.length) {
                const currentReading = mockSensorReadings[readingIndex++];
                setLatestReading(currentReading); // Keep this to update device readings in UI

                try {
                    // Send the raw reading to the backend for evaluation
                    const response = await axios.post(`${API_BASE_URL}/evaluate-reading`, currentReading);
                    const { evaluatedAlerts } = response.data;

                    // Dispatch the evaluated alerts to the reducer
                    dispatch({
                        type: 'PROCESS_READING',
                        payload: { evaluatedAlerts, alertIdCounter }, // Pass evaluated alerts
