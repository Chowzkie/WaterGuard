...
const initialDevices = [
  ...
  useEffect(() => {
    async function fetchDevices() {
      const response = await fetch('http://localhost:5000/devices');  // your backend API endpoint
...
function Dashboard() {
  ...
  return (
    <div className='component-wrapper-dashboard'>
      ...
      <DeviceStatus devicesData={devices} selectedDeviceId={selectedDeviceId} setSelectedDeviceId={setSelectedDeviceId} />
    </div>
  );
}
